...
ALTER TABLE POSITION DROP CONSTRAINT FK__POSITION__POS_NA__440B1D61;
...
-- drop PROCEDURE display_active_rooms
CREATE PROCEDURE display_active_rooms
AS
BEGIN
    SET NOCOUNT ON
    SELECT ri.ROOM_NO, rc.CLASS_NAME, rc.CLASS_TYPE, rc.PRICE
    -- , STRING_AGG(CONVERT(varchar, r.CHECK_IN, 104)+ ' - '+CONVERT(varchar, r.CHECK_OUT, 104), '; ') RSRV_DATES
    FROM ROOM_INFO ri
    ...
    GROUP BY ri.ROOM_NO, rc.CLASS_NAME,rc.CLASS_TYPE, rc.PRICE
    HAVING STRING_AGG(CONVERT(varchar, r.CHECK_IN, 104)+ ' - '+CONVERT(varchar, r.CHECK_OUT, 104), '; ') is NULL
END;
...
BEGIN
    ...
    -- also update rsrv status
    UPDATE RESERVATION SET [STATUS] = 'active' WHERE CHECK_IN between GETDATE()-1 and GETDATE()+2
END;
...
BEGIN
    ...
    SELECT 'Registration was successful'
END
...
from EMPLOYEE
EXEC register_user 
    @FULL_NAME='test', @PHONE_NUM='123', @CITY='-', @CREDIT_NUM='mastercard, 123', @CHECK_IN='2022-10-10', @CHECK_OUT='2022-10-20', @PAYMNT_TYPE='by cash'
...
BEGIN
    ...
    IF(@hour_left) <= 24
    BEGIN
        SELECT 'Request was denied. At least 24 hours left until the moment of check-in.'
    ...
    BEGIN
        ...
        BEGIN
            ...
            SELECT 'SUCCESS! RETURNED TOTAL PRICE: '+CONVERT(VARCHAR, @price)
        END
        ELSE SELECT 'SUCCESS! RETURNED TOTAL PRICE: 0'
    END
END

EXEC cancel_rsrv @RSRV_ID=20

update RESERVATION set [STATUS] = 'wait-list' where RSRV_ID = 24
...
alter table HOTEL_SERVICE drop constraint FK__HOTEL_SER__SERV___5165187F;
...
VALUES
    ...
    (14, '2022-04-25', '2022-05-22 01:00', '2022-05-24 12:00', 'online');
...
alter table EMP_JOURNAL drop constraint prevday;
...
begin
    ...
    where EMP_ID = @emp_id)
	begin
        update EMP_JOURNAL set C_Date = convert(date, getdate()), ARRV_TIME = @arrv_t, LEAV_TIME = @leav_t  
		where EMP_ID = @emp_id;
    end
	else begin
        insert into EMP_JOURNAL
        values(convert(date, getdate()), @emp_id, @arrv_t, @leav_t)
    end
    select 'Succesfully Updated(Inserted)'
end;
-- select * from EMP_JOURNAL;
exec BonusEmp @emp_id = 1, @arrv_t='8:00', @leav_t='18:00'

create trigger AddBEmp on EMP_JOURNAL
after update, insert 
as 
begin
    update emp set emp.BONUS = emp.BONUS + (empj.work_time_dur - 11) * (emp.SALARY*0.005)
	from EMPLOYEE emp
        join (select ej.EMP_ID, datediff(hour, ej.ARRV_TIME, ej.LEAV_TIME) as work_time_dur
        from EMP_JOURNAL ej join inserted insrt on insrt.EMP_ID = ej.EMP_ID) empj
...
-- New columns Bonus, Login, Password to EMPLOYEE table
alter table EMPLOYEE add BONUS decimal(10, 2);
update EMPLOYEE set BONUS = 0;

alter table EMPLOYEE add LOGIN varchar(255);
alter table EMPLOYEE add PASSWORD varchar(255);

update EMPLOYEE set LOGIN = lower(concat(substring(FULL_NAME, 1, 3), '_', substring(FULL_NAME, charindex(' ', FULL_NAME) + 1, 3))), 
	PASSWORD = lower(concat(substring(FULL_NAME, 1, 3), substring(FULL_NAME, charindex(' ', FULL_NAME) + 1, 3), convert(varchar, EMP_ID)));


select substring(emp.FULL_NAME, 1, charindex(' ', emp.FULL_NAME) - 1) as name
    , substring(emp.FULL_NAME, charindex(' ', emp.FULL_NAME) + 1, len(emp.FULL_NAME) - charindex(' ', emp.FULL_NAME)) as surname
    , p.POS_NAME
from EMPLOYEE emp
    join POSITION p on p.POS_ID = emp.POS_ID
where EMP_ID = 1;
...
create procedure getprice
    @phon_num varchar(25)
as begin
  select b.TOTAL_PRICE from GUEST g
  join RESERVATION rsrv on rsrv.GUEST_ID = g.GUEST_ID
  join BILL b on b.RSRV_ID = rsrv.RSRV_ID
  where rsrv.CHECK_IN > getdate() and g.PHONE_NUM = @phon_num
end;
